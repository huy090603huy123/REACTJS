...
export default function CreateRemedy() {
  ...
  const createRemedyImage = async () => {
    ...
    navigate('/admin-dashboard/doctor/manage-patient?date='+date, { replace: true });
  };
  ...
  const handlePushDrugToList=(drug)=>{
    ...
    const found = temp.find(ele => ele.id === drug.id);
                  ...
                  <div class="row">
                      {
                        listSeletedDrugs.map((drug)=>{
                          let foundUnit = units.find((unit)=>unit.key===drug.unit)
                          let displayedDrug=language==="vi" ? foundUnit.valueVi : foundUnit.valueEn;
                          return(
                              <div class="col-12 form-group">
                                <div class="row align-item-center text-center">
                                    ...
                                    <div class="col-2">
                                        <input readonly type="text" value={displayedDrug} class="form-control" placeholder="" />
